...
class KLSEScreener:
    ...
    def bursa_index(self) -> pandas.DataFrame:
        ...
        for row_index, row in dataframe.iterrows():
            ...
            series = pandas.Series(data=json.loads(s=df.to_json(orient="records", index=False))[0])
    ...
    def bursa_index_components(self) -> pandas.DataFrame:
        ...
        for row_index, row in dataframe.iterrows():
            if row["Code"].startswith("00"):
                ...
                components = ";".join([a.text for a in node.find_all("a")])
                dataframe.at[row_index, "Components"] = components
            elif row["Code"] in ("0200I"):
                df = self.fetch_html(url=row["Link"])[-1]
                components = ";".join(df["Name"].to_list())
                dataframe.at[row_index, "Components"] = components
            else:
                pass
        return dataframe
